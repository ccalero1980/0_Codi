...
/* ########################################################################## */
float    verlet_pt1_2filaments_rotacio(  vec_s  *r,
                      vec_s  *v,
                      vec_s  *fa,
                      vec_s  *vhydro,
                      param_s parameters, 
...
/* ########################################################################## */
{
  ...
  //  disp2_0        = (1.0 - 0.5*gamma_0*dt)*disp1_0;
    ...
    if (parameters.oseen ==1){
      oseen_2filaments_rotacio(r, fa, v_h, parameters, taupara);
    }
    if (parameters.blake ==1){
      blake_2filaments_rotacio(r, fa, v_h, parameters, taupara);
    }
...
/* ########################################################################## */
float    verlet_pt1_2filaments_gamma_var(  vec_s  *r,
                      vec_s  *v,
                      vec_s  *fa,
                      param_s parameters )
  ...
  //  disp2_0        = (1.0 - 0.5*gamma_0*dt)*disp1_0;
   ...
   //printf("0: r[0].x = %le\n", r[0].x );
    ...
    for( i = 0; i < (n_mon1 + n_mon2); i++ )
    {
      ...
      if(i==0){gamma=10*parameters.gamma1;}
...
/* ########################################################################## */
void      verlet_pt2_2filaments_rotacio(  vec_s  *r,
                       vec_s  *v,
                       vec_s  *fa,
                       vec_s  *vhydro,
                       vec_s  *vhydro1,
...
/* ########################################################################## */
{
  ...
  vec_s f_i, v_norm, v_hydro;
    ...
    if (parameters.oseen ==1){
      oseen_2filaments_rotacio(r, fa, v_hyd, parameters, taupara);
    }
    if (parameters.blake ==1){
//      blake_2filaments_rotacio(r, fa, v_hyd, parameters, taupara);
      blake_2filaments_rotacio2(r, fa, v_hyd, parameters, taupara, vhydro1, vhydro2);
    }
...
/* ########################################################################## */
void      verlet_pt2_2filaments_gamma_var(  vec_s  *r,
                       vec_s  *v,
                       vec_s  *fa,
                       param_s parameters )
...
void     oseen_2filaments_rotacio( vec_s *r,
    vec_s *fa,
    vec_s *v_h,
    param_s parameters,
    float taupara)
...
//      v_h[i].z = (0.75 * v_hydr.z ) *  (hydr_radius/ gamma);
      ...
      if(i!=n_mon1){
          ...
          rijx     = r[i].x - r[n_mon1].x;
          rijy     = r[i].y - r[n_mon1].y;
          rijz     = r[i].z - r[n_mon1].z;
          mag_ij  = sqrt(rijx*rijx + rijy*rijy + rijz*rijz);  
          fac = taupara/((8.*PI*eta)*mag_ij*mag_ij*mag_ij);
          v_h[i].x += -fac*rijz;
          v_h[i].y += 0.0;
          v_h[i].z +=  fac*rijx;   
      }
...
void     blake_2filaments_rotacio( vec_s *r,
    vec_s *fa,
    vec_s *v_h,
    param_s parameters, 
    float taupara)
/*  ************************************************************************ */
{
  ...
  float rijx, rijy,rijz,rijxbar, rijybar,rijzbar, fac;
  float mag_ij, mag_ij2, mag_ij3, mag_ijbar, mag_ijbar2, mag_ijbar3, mag_ijbar5;
  int  i,j,k; 
  vec_s r_i, r_j, f_i, f_j, v_hydr;
...
//    mag_ijbar  /= bond;
    ...
    if(j != i)
      ...
      if(i!=n_mon1){
          ...
          rijxbar     = r[i].x - r[n_mon1].x;
          rijybar     = r[i].y - r[n_mon1].y;
          rijzbar     = r[i].z + r[n_mon1].z;

          mag_ij2  = (rijx*rijx + rijy*rijy + rijz*rijz);
          mag_ij = sqrt(mag_ij2);
          mag_ij3 = mag_ij2*mag_ij;
          mag_ijbar2  = rijx*rijx + rijy*rijy + rijzbar*rijzbar;
          mag_ijbar  = sqrt(mag_ijbar2);
          mag_ijbar3 = mag_ijbar*mag_ijbar2;
          mag_ijbar5 = mag_ijbar3*mag_ijbar2;
          ...
          fac = taupara/(8.*PI*eta);
          v_h[i].x += fac*((rijz*(1./mag_ijbar3 - 1./mag_ij3) - 6.*rijx*rijx*r[i].z/mag_ijbar5));
          v_h[i].y += 0.0;
          v_h[i].z +=  fac*(rijx*(1./mag_ij3 - 1./mag_ijbar3) - 6.*rijx*rijzbar*r[i].z/mag_ijbar5);   
      }
...
void     blake_2filaments_rotacio2( vec_s *r,
    vec_s *fa,
    vec_s *v_h,
    param_s parameters, 
    float taupara,
...
//    mag_ijbar  /= bond;
    ...
    if(j != i)
      ...
      vhydro1[i].x = ( v_hydr.x ) *  (1./(8.*PI*eta));
      vhydro1[i].y = ( v_hydr.y ) *  (1./(8.*PI*eta));
      vhydro1[i].z = ( v_hydr.z ) *  (1./(8.*PI*eta));
      ...
      if(i!=n_mon1){
          ...
          fac = taupara/(8.*PI*eta);
          vhydro2[i].x = fac*((rijz*(1./mag_ijbar3 - 1./mag_ij3) - 6.*rijx*rijx*r[i].z/mag_ijbar5));
          vhydro2[i].y = 0.0;
          vhydro2[i].z = fac*(rijx*(1./mag_ij3 - 1./mag_ijbar3) - 6.*rijx*rijzbar*r[i].z/mag_ijbar5);
