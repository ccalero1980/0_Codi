...
/* ########################################################################## */
{
  ...
  float ddx, ddy, ddz, gammapara, gammaferro, gammacirc, gam;
  int  i;
  vec_s f_i, v_norm, v_hydr;
    ...
    if( init_flag )
    {
      ...
      dt             = parameters.dt;
      bond           = parameters.b_l;
      gammapara =  6.0*PI*parameters.rpara*parameters.viscosity;
      gammaferro = 6.0*PI*parameters.rferro*parameters.viscosity;
      gammacirc =  6.0*PI*parameters.rcirc*parameters.viscosity;
      massa          = parameters.mass;
      disp1          = 1.0/(1.0 + 0.5*gamma*dt);
      n_fil          = parameters.n_fil;
      n_mon          = parameters.n_mon;
  ...
  else if(i==n_mon/2){gam = gammaferro;}
  else{gam = gammacirc;}
  disp1    = 1.0/(1.0 + 0.5*gam*dt);
...
/* ########################################################################## */
{
  ...
  static int n_mon, n_fil;
  int  i;
  float gam, gammapara, gammaferro, gammacirc;
